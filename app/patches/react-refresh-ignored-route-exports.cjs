...
var buildReactRefreshIgnoredRouteExportsStatements = _babel_template.statements(
  `
const tsrHot = import.meta.hot
if (tsrHot && typeof window !== 'undefined') {
  tsrHot.data ??= {}
 ...
 */
var buildRefreshAnchorStatement = _babel_template.statement(
...
function createReactRefreshIgnoredRouteExportsPlugin() {
  return {
    ...
    onAddHmr(ctx) {
      const anchorName = require_utils.getUniqueProgramIdentifier(
        ctx.programPath,
